variable "create_resource_group" {
  description = "Flag to suppress the automatic creation of the resource group within this module."
  type        = bool
  default     = true
}
...
variable "server_name" {
  type        = string
  description = <<EOS
    (Required) The prefix of the mssql server name.
    The final name would be
      primary server:     <var.server_name>-<var.location>.database.windows.net
      secondary server:   <var.server_name>-<var.failover_server_location>.database.windows.net
      failover endpoint:  <var.server_name>.database.windows.net
    This variable takes no effect when var.server_names_override is provided but remains required.
EOS
}

variable "server_names_override" {
  type = object({
    primary_server_name    = string
    secondary_server_name  = optional(string)
    failover_endpoint_name = optional(string)
  })
  default     = { primary_server_name = "" }
  description = <<EOS
    (Optional) To provide a override for the full server name to avoid using the default naming convention for var.server_name.
    Please DO NOT provide a value for this variable and use var.server_name for new servers.
    It is not recommended to use this variable. It is only for special situations like importing an existing server when recreating is impossible.
    The final name would be
      primary server:     <var.server_names_override.primary_server_name>.database.windows.net
      secondary server:   <var.server_names_override.secondary_server_name>.database.windows.net
      failover endpoint:  <var.server_names_override.failover_endpoint_name>.database.windows.net
EOS
}
...
variable "server_tags" {
  type        = map(string)
  description = "Tags for mssql servers"
  default     = {}
}
...
variable "elastic_pools" {
  ...
  Config keys:
    enable_zone_redundant = (Optional) Whether or not this elastic pool is zone redundant. Defaults to false.
                                       SKU tier needs to be Premium for DTU based or BusinessCritical for vCore based sku.
                                       Subject to availability in regions listed in https://docs.microsoft.com/en-ca/azure/availability-zones/az-region#azure-regions-with-availability-zones
    max_size_gb           = (Optional) The max data size of the elastic pool in gigabytes. Defaults to the max size of the sku.
...
variable "failover_server_location" {
  type        = string
  default     = ""
  description = "The location of the failover server. Must be different than the var.location"
}

variable "failover_grace_minutes" {
  type        = number
  description = "The grace period in minutes before failover with data loss is attempted. Minimum is 60."
  default     = 60
  validation {
    condition     = var.failover_grace_minutes >= 60
    error_message = "The grace period for failover must be greater than 60 minutes."
  }
}
...
variable "failover_group_tags" {
  type        = map(string)
  default     = {}
  description = "A map for the failover group azure resource tags."
}
